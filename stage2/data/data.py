...
def amplitude_freq_generation(dim,floor_amplitude=0.1):
    ...
    f1=(np.random.rand(*dim)-0.5) * 60
    f2=(np.random.rand(*dim)-0.5) * 100
    amp=np.random.rand(*dim)*(1-0.01)+0.01
    f3 = (np.random.rand(*dim) - 0.5) * 256
    return f1,f2,amp,f3
...
def gen_signal(num_samples, signal_dim, num_freq, min_sep, distance='normal', amplitude='normal_floor',
    ...
    for n in range(num_samples):
        ...
        for i in range(nfreq[n]):
            ...
            sin=amp[n,i]*np.exp(2j*np.pi*(r[n,i]*np.cos(f1[n,i]*xgrid_t.T+theta[n,i])+f3[n,i]*xgrid_t.T+f2[n,i]*np.power(xgrid_t.T,2.)))
            s[n, 0] = s[n, 0] + sin.real
            s[n, 1] = s[n, 1] + sin.imag

        s[n] = s[n] / (np.sqrt(np.mean(np.power(s[n], 2)))+1e-13)
    sig_details=np.concatenate([f1, f2,r,theta,amp,start_pos.astype('int'),sig_len,f3],axis=-1,dtype=np.float32)
    return s.astype('float32'),nfreq,sig_details
