...
import torch
def custom_stft(x, n_fft, winlen):
    """
    复数信号STFT实现
    输入：x - (batch, T, 2)
    输出：(batch, n_fft, T, 2)
    ...
    # 创建滑动窗口（时间复杂度O(T*n_fft)）
    windows = x.unfold(1, winlen,1)  # (batch, T, fft)

    # 汉宁窗与FFT计算
    window = torch.hamming_window(winlen, device=x.device)
    pad = (0, n_fft-winlen)
    segment_sigs=windows * window
    segment_sigs=F.pad(segment_sigs, pad)
    stft = torch.fft.fftshift(torch.fft.fft(segment_sigs, dim=2),dim=2)#b,t,f
    # plt.imshow((torch.abs(stft[0].permute(1, 0))))

    # 重组维度并分离实虚部
    return stft.permute(0, 2, 1)


def custom_istft(spec, n_fft, orig_len, winlen):
    """
    复数频谱ISTFT实现
    输入：X - (batch, n_fft, T, 2)
    输出：(batch, orig_len, 2)
    """
    batch_size, _, num_frames = spec.shape

    # 频率维度转置与时域变换
    win=torch.hamming_window(winlen, device=spec.device)
    spec = spec.permute(0, 2, 1)# (batch, T, n_fft)
    spec=torch.fft.fftshift(spec,dim=2)
    windows = torch.fft.ifft(spec, dim=2)[...,:winlen] * win

    # 重叠相加重构（空间复杂度O(B*(T+n_fft))）
    output = torch.zeros(batch_size, num_frames + winlen - 1, dtype=torch.complex64, device=spec.device)
    weights = torch.zeros_like(output.real)

    for t in range(num_frames):
        output[:, t:t + winlen] += windows[:, t]
        weights[:, t:t + winlen] += win

        # 归一化与裁剪
    reconstructed = output / weights
    return reconstructed#reconstructed[:, (winlen-1) // 2:(winlen-1) // 2 + orig_len]

class ComplexConv1d_dilated(Module):

    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0,
                 dilation=1, groups=1, bias=True):
        super(ComplexConv1d_dilated, self).__init__()
...
class ComplexConvTranspose2d(Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0,
        ...
        self.conv_tran_i = ConvTranspose2d(in_channels, out_channels, kernel_size, stride, padding,
                                           output_padding, groups, bias, dilation, padding_mode)
        nn.init.xavier_uniform_(self.conv_tran_r.weight)
        nn.init.xavier_uniform_(self.conv_tran_i.weight)
...
def apply_complex4(fr, fi, input):
    ...
    inp_real,inp_imag=torch.chunk(input,2,-1)
    ret_real=(fr(inp_real.squeeze(-1)) - fi(inp_imag.squeeze(-1)))
    ret_imag=(fr(inp_imag.squeeze(-1)) + fi(inp_real.squeeze(-1)))
    ret=torch.cat((ret_real.unsqueeze(-1),ret_imag.unsqueeze(-1)),-1)
    return ret
...
class ComplexLinear(Module):
    ...
    def forward(self, input):
        return apply_complex4(self.fc_r, self.fc_i, input)
